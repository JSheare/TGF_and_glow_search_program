...
def search(unit, checked_dates, data_path, results_path, autosearch_path):
    ...
    for day in queue:
        ...
        if result.wait() != 0:
            ...
            tl.make_path(f'{autosearch_path}/Error Logs')
            with open(f'{autosearch_path}/Error Logs/{unit}_{day}_Error.txt', 'w') as error_file:
            ...
            with open(f'{autosearch_path}/checked_dates.json', 'w') as date_file:
...
def main():
    ...
    try:
        with open(f'{autosearch_path}/pid.txt', 'r') as existing_pid_file:
    ...
    # Runs the program normally if it isn't running already
        ...
        with open(f'{autosearch_path}/pid.txt', 'w') as pid_file:
            pid_file.write(str(os.getpid()))

        try:
            with open(f'{autosearch_path}/checked_dates.json', 'r') as date_file:
        ...
        os.remove(f'{autosearch_path}/pid.txt')
